...
router.post("/:id/info", async (req, res) => {
    ...
    try {
        const { rows } = await db.query("INSERT INTO user_info (fullname, email, user_id) VALUES ($1, $2, $3) RETURNING *;", [fullname, email, id]);
        res.status(201).json(rows[0]);
    } catch (err) {
        console.error(err);
        res.status(500).send("Server error.");
...
router.put("/:id/info", async (req, res) => {
    ...
    const { fullname, email } = req.body;
    try {
        await db.query("UPDATE user_info SET fullname = $1, email = $2 WHERE user_id = $3;", [fullname, email, id]);
